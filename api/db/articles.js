...
exports.articles_searchBy_title =  async (title) => {
    ...
    //returns all the articles inside the promise
    return client.query('SELECT articles.article_id,articles.title,articles.thumbnail,articles.creator_id,articles.createdat,creators.creator_id,creators.name,creators.profileurl FROM articles INNER JOIN creators ON articles.creator_id = creators.creator_id WHERE articles.title=' +
     title);
}